...
TaskHandle_t TaskBlink_handle; // You can (don't have to) use this to be able to manipulate a task from somewhere else.
...
void setup() {
  ...
  xTaskCreate(
    ...
    ,  1  // Priority
    ,  &TaskBlink_handle // Task handle is not used here - simply pass NULL
    );
  ...
  Serial.printf("Basic RTOS Multi Threading \n");
...
void loop(){
  if(analog_read_task_handle != NULL){ // Make sure that the task actually exists
    ...
    vTaskDelete(TaskBlink_handle); // Delete task
    analog_read_task_handle = NULL; // prevent calling vTaskDelete on non-existing task
    TaskBlink_handle = NULL;

    Serial.printf("both tasks are deleted by their handlers after 40 seconds");
  }
}
